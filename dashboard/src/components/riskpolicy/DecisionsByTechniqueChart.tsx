...
interface DecisionsByTechniqueChartProps {
  data: RiskPolicyData['decisionsByTechnique'] | null;
  loading: boolean;
}

export const DecisionsByTechniqueChart: React.FC<DecisionsByTechniqueChartProps> = ({ data, loading }) => {
  const hasData = data && data.length > 0;

  return (
    <div className="glass-panel p-5 h-[350px] flex flex-col">
      <h3 className="text-lg font-semibold mb-6 text-white">Policy Decisions by Technique</h3>
        ...
        {loading ? (
          ...
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={data} layout="vertical" margin={{ top: 0, right: 10, left: 10, bottom: 0 }}>
              ...
              <Bar dataKey="allowed" name="Allowed" stackId="a" fill="#10b981" radius={[0, 4, 4, 0]} />
            </BarChart>
          </ResponsiveContainer>
        )}
      </div>
