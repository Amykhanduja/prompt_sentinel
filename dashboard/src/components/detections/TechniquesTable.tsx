import React, { useState } from 'react';
import { DetectionsData } from '../../services/detectionsApi';
import { motion } from 'framer-motion';
import { ArrowUpDown, ShieldAlert } from 'lucide-react';

interface TechniquesTableProps {
  data: DetectionsData['techniquesTable'] | null;
  loading: boolean;
  onRowClick: (techniqueId: string) => void;
}

type SortKey = keyof DetectionsData['techniquesTable'][0];

export const TechniquesTable: React.FC<TechniquesTableProps> = ({ data, loading, onRowClick }) => {
  const [sortKey, setSortKey] = useState<SortKey>('detectionCount');
  const [sortOrder, setSortOrder] = useState<'asc' | 'desc'>('desc');

  const handleSort = (key: SortKey) => {
    if (sortKey === key) {
      setSortOrder(sortOrder === 'asc' ? 'desc' : 'asc');
    } else {
      setSortKey(key);
      setSortOrder('desc');
    }
  };

  const sortedData = React.useMemo(() => {
    if (!data) return [];
    return [...data].sort((a, b) => {
      const aVal = a[sortKey];
      const bVal = b[sortKey];
      if (aVal < bVal) return sortOrder === 'asc' ? -1 : 1;
      if (aVal > bVal) return sortOrder === 'asc' ? 1 : -1;
      return 0;
    });
  }, [data, sortKey, sortOrder]);

  return (
    <div className="glass-panel p-5 overflow-hidden flex flex-col h-full">
      <div className="flex justify-between items-center mb-4">
        <h3 className="text-lg font-semibold text-white">Technique Analysis</h3>
      </div>
      
      <div className="flex-1 overflow-x-auto relative">
        {loading ? (
          <div className="space-y-3">
            {[1, 2, 3, 4, 5].map((i) => (
              <div key={i} className="h-12 bg-white/5 rounded-lg animate-pulse w-full"></div>
            ))}
          </div>
        ) : !data || data.length === 0 ? (
          <div className="absolute inset-0 flex flex-col items-center justify-center text-gray-500">
            <ShieldAlert className="w-12 h-12 mb-2 opacity-50" />
            <p>No Data Available</p>
          </div>
        ) : (
          <table className="w-full text-sm text-left">
            <thead className="text-xs text-gray-400 uppercase bg-black/20 sticky top-0 backdrop-blur-sm z-10 rounded-t-lg">
              <tr>
                {[
                  { key: 'techniqueId', label: 'Technique' },
                  { key: 'detectionCount', label: 'Count' },
                  { key: 'avgConfidence', label: 'Avg Confidence' },
                  { key: 'avgRiskScore', label: 'Avg Risk Score' },
                  { key: 'detectorType', label: 'Primary Detector' },
                  { key: 'percentageOfTotal', label: '% of Total' },
                ].map((col) => (
                  <th 
                    key={col.key} 
                    className="px-4 py-3 font-medium cursor-pointer hover:text-white transition-colors group select-none"
                    onClick={() => handleSort(col.key as SortKey)}
                  >
                    <div className="flex items-center gap-1">
                      {col.label}
                      <ArrowUpDown className={`w-3 h-3 ${sortKey === col.key ? 'text-primary' : 'opacity-0 group-hover:opacity-50'}`} />
                    </div>
                  </th>
                ))}
              </tr>
            </thead>
            <tbody>
              {sortedData.map((item, index) => (
                <motion.tr 
                  key={item.techniqueId}
                  initial={{ opacity: 0, x: -10 }}
                  animate={{ opacity: 1, x: 0 }}
                  transition={{ delay: index * 0.05 }}
                  onClick={() => onRowClick(item.techniqueId)}
                  className="border-b border-white/5 hover:bg-white/5 transition-colors cursor-pointer group"
                >
                  <td className="px-4 py-3">
                    <div className="flex flex-col">
                      <span className="text-white font-medium group-hover:text-primary transition-colors">{item.techniqueName}</span>
                      <span className="text-xs text-gray-500 font-mono">{item.techniqueId}</span>
                    </div>
                  </td>
                  <td className="px-4 py-3 text-gray-300 font-mono">{item.detectionCount}</td>
                  <td className="px-4 py-3">
                    <div className="w-full bg-white/10 rounded-full h-1.5 mb-1 mt-1.5">
                      <div className="bg-primary h-1.5 rounded-full" style={{ width: `${item.avgConfidence * 100}%` }}></div>
                    </div>
                    <span className="text-xs text-gray-400">{(item.avgConfidence * 100).toFixed(0)}%</span>
                  </td>
                  <td className="px-4 py-3 text-center">
                    <div className={`inline-flex items-center justify-center w-8 h-8 rounded-full border font-bold text-xs ${item.avgRiskScore >= 80 ? 'text-danger bg-danger/10 border-danger/20' : item.avgRiskScore >= 50 ? 'text-warning bg-warning/10 border-warning/20' : 'text-success bg-success/10 border-success/20'}`}>
                      {item.avgRiskScore.toFixed(0)}
                    </div>
                  </td>
                  <td className="px-4 py-3 text-gray-300 capitalize">{item.detectorType}</td>
                  <td className="px-4 py-3 text-gray-400 text-xs font-mono">{item.percentageOfTotal.toFixed(1)}%</td>
                </motion.tr>
              ))}
            </tbody>
          </table>
        )}
      </div>
    </div>
  );
};
