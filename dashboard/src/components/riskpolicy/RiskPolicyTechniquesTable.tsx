...
interface RiskPolicyTechniquesTableProps {
  data: RiskPolicyData['techniquesTable'] | null;
  loading: boolean;
}

type SortKey = keyof RiskPolicyData['techniquesTable'][0];

export const RiskPolicyTechniquesTable: React.FC<RiskPolicyTechniquesTableProps> = ({ data, loading }) => {
  const [sortKey, setSortKey] = useState<SortKey>('avgRisk');
  ...
  return (
    <div className="glass-panel p-5 overflow-hidden flex flex-col h-full">
      <div className="flex justify-between items-center mb-4">
        <h3 className="text-lg font-semibold text-white">Risk & Policy Technique Analysis</h3>
      </div>
            ...
            <thead className="text-[11px] text-gray-400 uppercase bg-black/20 sticky top-0 backdrop-blur-sm z-10 rounded-t-lg">
              <tr>
                {[
                  ...
                  { key: 'decisionCount', label: 'Decision Count' },
                  { key: 'avgConfidence', label: 'Avg Confidence' },
                  { key: 'avgPolicyScore', label: 'Avg Policy Score' },
                ].map((col) => (
                  <th 
                    key={col.key} 
            ...
            <tbody>
              {sortedData.map((item, index) => (
                ...
                >
                  ...
                  <td className={`px-3 py-3 font-mono ${getRiskColor(item.avgRisk)}`}>{item.avgRisk.toFixed(1)}</td>
                  <td className={`px-3 py-3 font-mono ${getRiskColor(item.highestRisk)}`}>{item.highestRisk.toFixed(1)}</td>
                  <td className={`px-3 py-3 font-mono ${getRiskColor(item.lowestRisk)}`}>{item.lowestRisk.toFixed(1)}</td>
                  <td className="px-3 py-3 text-gray-300 font-mono">{item.decisionCount}</td>
                  <td className="px-3 py-3 text-purple-400 font-mono">{(item.avgConfidence * 100).toFixed(1)}%</td>
                  <td className="px-3 py-3 text-cyan-400 font-mono font-bold">{item.avgPolicyScore.toFixed(2)}</td>
                </motion.tr>
              ))}
            </tbody>
          </table>
